...
<template>
  ...
  >
    ...
    <main class="relative z-20 w-full max-w-xl mx-auto space-y-3.5 my-auto">
      ...
      <!-- 3. Ringkasan XP Ormawa Expo (Hari ke-3) -->
      <div class="bg-[#19110a]/95 backdrop-blur-md border border-[#8b6f4e] rounded-xl p-3.5 sm:p-4 space-y-3 shadow-md">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h3 class="font-pixel text-xs sm:text-sm font-bold text-[#f0d060]">PROGRES ORMAWA EXPO</h3>
            <p class="font-sans text-[10px] text-[#c4956a] mt-0.5">PIC stan memindai QR profilmu setelah misi selesai.</p>
          </div>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
          <div class="rounded-xl border border-[#22c55e] bg-[#102315] p-3">
            <span class="text-[8px] font-sans uppercase text-[#86efac]">XP Ormawa</span>
            <strong class="block font-pixel text-base text-[#facc15] mt-1">+{{ gameStore.ormawaXpEarned }}</strong>
          </div>
          <div class="rounded-xl border border-[#facc15] bg-[#2a1a0e] p-3">
            <span class="text-[8px] font-sans uppercase text-[#d8b878]">XP Total</span>
            <strong class="block font-pixel text-base text-white mt-1">{{ gameStore.participant.totalXp.toLocaleString('id-ID') }}</strong>
          </div>
          <div class="col-span-2 sm:col-span-1 rounded-xl border border-[#8b6f4e] bg-[#170f07] p-3">
            <span class="text-[8px] font-sans uppercase text-[#c4956a]">Aktivitas</span>
            <strong class="block font-pixel text-[10px] text-[#f0d060] mt-1">{{ gameStore.visitedOrmawaCount }} STAMP · {{ gameStore.interestCount }} MINAT</strong>
          </div>
        </div>
        <RouterLink to="/ormawa" class="block">
          <button type="button" @click="() => safeSound(() => soundEngine.playClick?.())" class="rpg-btn-primary w-full py-2 px-3 text-[10px] font-pixel font-bold cursor-pointer">BUKA KATALOG & QR PROFIL</button>
        </RouterLink>
      </div>
