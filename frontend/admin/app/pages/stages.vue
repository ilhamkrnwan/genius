          ...
          >
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#271d15] pb-2.5">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <!-- Format -->
                <span class="px-2 py-0.5 rounded text-[10px] font-mono border border-[#3d2d1e] bg-[#1a140f] text-gray-300">
                  {{ item.format }}
                </span>
              </div>
            ...
            <div class="space-y-1">
              <h3 class="font-sans text-base sm:text-lg font-bold text-white group-hover:text-amber-300 transition-colors">
                {{ item.name }}
              </h3>
...
async function syncOfficialRoadmap() {
  ...
  try {
    ...
    if (res.success) {
      await loadAllData();
      toast.success("Roadmap Tersinkronisasi", res.message || "Berhasil menyinkronkan 3-Day Event Roadmap & Rundown resmi!");
    }
  } catch (err: any) {
    toast.error("Gagal Sinkronisasi Roadmap", err.message || "Pastikan backend aktif.");
  } finally {
    syncing.value = false;
  }
}
...
async function confirmStageTransition() {
  ...
  const target = stages.value.find((s) => s.id === targetTransitionStageId.value);
  const confirmed = await confirmModal.show({
    title: "Aktifkan Babak Kegiatan?",
    description: `Apakah Anda yakin ingin mengalihkan tahapan aktif ke '${target?.name || 'Tahapan Terpilih'}'? Seluruh aktivitas pos, presensi, dan misi akan disesuaikan dengan tahapan ini.`,
  ...
  try {
    ...
    if (res.success) {
      ...
      await fetchStages();
      toast.success("Babak Diaktifkan", res.message || `'${target?.name || 'Tahapan'}' kini berstatus aktif.`);
    }
  } catch (err: any) {
    toast.error("Gagal Mengaktifkan Babak", err.message || "Terjadi kesalahan sistem.");
  } finally {
    saving.value = false;
  }
}
...
function exportRundownCsv() {
  try {
    ...
    toast.success("Ekspor CSV Berhasil", "File CSV Rundown resmi telah diunduh.");
  } catch (err: any) {
    toast.error("Gagal Ekspor CSV", err.message || "Terjadi kesalahan saat mengekspor.");
  }
}
