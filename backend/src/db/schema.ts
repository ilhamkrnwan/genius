...
export const ormawaBooths = pgTable("ormawa_booths", {
  ...
  stampInstructions: jsonb("stamp_instructions").$type<string[]>().notNull().default([]),
  qrCode: varchar("qr_code", { length: 255 }).notNull().unique(),
  xpReward: integer("xp_reward").notNull().default(2),
...
export const ormawaScans = pgTable("ormawa_scans", {
  ...
  xpEarned: integer("xp_earned").notNull().default(2),
...
export const ormawaInterests = pgTable("ormawa_interests", {
  ...
  xpBonusEarned: integer("xp_bonus_earned").notNull().default(3),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
